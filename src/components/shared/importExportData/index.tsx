...
const ImportExportButton = (props: ImportExportProps) => {
  ...
  const client = apolloClientSingleton.getInstance(props.token);

  return (
    <span
      className="flex flex-row items-center cursor-pointer"
      onClick={() => entityType[type] && entityType[type](client)}
    >
...
const ImportExport = (props: ImportExportProps) => {
  ...
  const Import = (
    <Modal trigger={Export} title="Import" content={<div>Import</div>} />
  );
